 ...
 * 5) Equipment opt-in (OTA / existing users) → /welcome-equipment
...
export default function AppIndex() {
  ...
  useEffect(() => {
    if (!session || needsCrewSetup || crewLoading || needsEquipmentPrompt || equipmentLoading) {
  ...
  }, [session, needsCrewSetup, crewLoading, needsEquipmentPrompt, equipmentLoading]);

  if (
    isPending ||
    preAuthLoading ||
    (session && (crewLoading || notifLoading || equipmentLoading))
  ) {
    return (
