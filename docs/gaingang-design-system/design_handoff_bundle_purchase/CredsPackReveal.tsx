...
import { CREDS_DEEP, CREDS_GOLD } from '@/lib/shop';
...
const PANEL_W = 322;
const CY = 0.4; // fraction of screen height — vertical anchor of the reveal
...
export type CredsPackRevealProps = {
  ...
  amount: number;
  /** e.g. "$9.99" (CREDS_PACKS[n].priceLabel) */
  priceLabel: string;
...
export function CredsPackReveal({
  ...
  return (
    <View style={styles.root} pointerEvents="box-none">
      ...
      <View style={[styles.centerLayer, { top: `${CY * 100}%` }]} pointerEvents="none">
      ...
      <Animated.View style={[styles.panel, { top: `${CY * 100}%` }, panelStyle]}>
        ...
        <View style={styles.body}>
          <Animated.Text style={[styles.kicker, kickerStyle]}>PACK UNLOCKED</Animated.Text>
...
function ChargingCoin({ clock }: { clock: SharedValue<number> }) {
    ...
    return (
      <View
        ...
        style={{
          ...
          transformOrigin: `0.75px ${SZ / 2 + 6}px`,
        }}
      />
    );
  });
  ...
  return (
    <Animated.View style={[{ position: 'absolute', width: SZ, height: SZ, marginLeft: -SZ / 2, marginTop: -SZ / 2, alignItems: 'center', justifyContent: 'center' }, coinStyle]}>
      <Animated.View style={[{ position: 'absolute', width: SZ + 68, height: SZ + 68, borderRadius: (SZ + 68) / 2, backgroundColor: GOLD }, auraStyle]} />
      <Animated.View style={[{ position: 'absolute', width: SZ + 12, height: SZ + 12, borderRadius: (SZ + 12) / 2, borderWidth: 1.5, borderColor: `rgba(${GOLD_RGB},0.5)` }, spinOuter]}>
        {ticks}
      </Animated.View>
      <Animated.View style={[{ position: 'absolute', width: SZ - 14, height: SZ - 14, borderRadius: (SZ - 14) / 2, borderWidth: 1.5, borderStyle: 'dashed', borderColor: `rgba(${GOLD_RGB},0.6)` }, spinInner]} />
      <CredPlate size={SZ * 0.72} glow />
    </Animated.View>
  );
}

// ─── Falling coin / aura confetti — continuous celebration after the burst ──
function Confetti({ clock }: { clock: SharedValue<number> }) {
  return (
    <View style={StyleSheet.absoluteFill} pointerEvents="none">
      {Array.from({ length: 20 }, (_, i) => (
        <ConfettiPiece key={i} clock={clock} i={i} />
      ))}
    </View>
  );
}

function ConfettiPiece({ clock, i }: { clock: SharedValue<number>; i: number }) {
  ...
  const style = useAnimatedStyle(() => {
    ...
    const y = interpolate(lp, [0, 1], [-40, 820], Extrapolation.CLAMP);
...
const styles = StyleSheet.create({
  ...
  centerLayer: { position: 'absolute', left: '50%', width: 0, height: 0, alignItems: 'center', justifyContent: 'center' },

  shock: {
    position: 'absolute', width: 200, height: 200, marginLeft: -100, marginTop: -100,
    borderRadius: 100, borderWidth: 2, borderColor: GOLD, shadowColor: GOLD,
  ...
  body: { paddingHorizontal: 18, paddingTop: 20, paddingBottom: 18, alignItems: 'center', gap: 8 },
  ...
  ctaHit: { marginTop: 16, width: '100%', borderRadius: 13, overflow: 'hidden', shadowColor: AURA1, shadowOffset: { width: 0, height: 8 } },
  cta: { height: 48, alignItems: 'center', justifyContent: 'center' },
