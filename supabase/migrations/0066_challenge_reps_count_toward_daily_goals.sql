-- Challenge / war reps count toward matching daily-goal exercises when that
-- movement is on the user's plan for the day. Feed cards stay a plain
-- consolidated exercise list — no "Gang War" / "Weekly Challenge" notes.

-- ----------------------------------------------------------------------------
-- Helper: sync challenge/war amount into daily goals (preferred) or freeform
-- ----------------------------------------------------------------------------
...
declare
  ...
  v_yesterday date;
  v_match record;
  v_existing_amount numeric(10, 1);
...
begin
  ...
  -- Prefer writing into today's matching daily-goal exercises across the
  -- user's gangs so challenge/war work advances daily progress.
  for v_match in
    select
    ...
    where dg.goal_date = p_activity_date
      and dge.unit = p_unit
      and (
        (p_exercise_id is not null and dge.exercise_id = p_exercise_id)
        or lower(trim(e.name)) = lower(trim(coalesce(p_exercise_name, '')))
      )
  loop
    v_matched := true;
    ...
    where a.user_id = p_user_id
      and a.daily_goal_id = v_match.daily_goal_id
    limit 1;
    ...
    if p_gang_id is not null and v_match.gang_id = p_gang_id then
      v_preferred_activity_id := v_activity_id;
    elsif v_preferred_activity_id is null then
      v_preferred_activity_id := v_activity_id;
    end if;

    select ae.id, ae.amount
    into v_exercise_row_id, v_existing_amount
    from public.activity_exercises ae
    where ae.activity_id = v_activity_id
      and ae.daily_goal_exercise_id = v_match.daily_goal_exercise_id
    limit 1;

    if v_exercise_row_id is null then
      ...
      values (
        v_activity_id,
        p_exercise_id,
        coalesce(nullif(trim(v_match.exercise_name), ''), p_exercise_name, 'Exercise'),
        coalesce(v_match.exercise_category, p_category),
        v_match.unit,
        v_amount,
        null,
        v_match.daily_goal_exercise_id
      );
    else
      update public.activity_exercises
      set
        amount = greatest(coalesce(v_existing_amount, 0), v_amount),
        exercise_id = coalesce(p_exercise_id, exercise_id),
        exercise_name = coalesce(nullif(trim(v_match.exercise_name), ''), exercise_name),
        category = coalesce(v_match.exercise_category, category),
        notes = null,
        updated_at = now()
      where id = v_exercise_row_id;
    end if;
  end loop;
  ...
  if v_matched then
    ...
    set
      ...
      current_streak = greatest(coalesce(p.current_streak, 0), coalesce(
        (select a.streak_at_log from public.activities a where a.id = v_preferred_activity_id),
        1
      )),
    ...
    return v_preferred_activity_id;
  end if;

  -- No matching daily exercise — keep a freeform feed row, without source tags.
  ...
  set
    last_active_on = greatest(coalesce(p.last_active_on, p_activity_date), p_activity_date),
    current_streak = greatest(coalesce(p.current_streak, 0), coalesce(
      (select a.streak_at_log from public.activities a where a.id = v_activity_id),
      1
    )),
    longest_streak = greatest(
      coalesce(p.longest_streak, 0),
...
-- ----------------------------------------------------------------------------
-- Gang war: sync contribution with no source tag
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Weekly challenge: sync today's attempt score (not weekly best) with no tag
-- ----------------------------------------------------------------------------
...
begin
  ...
  -- Sync today's submitted score so daily goals reflect work done today.
  v_activity_id := public.sync_tagged_activity_exercise(
    v_user_id,
    v_local_date,
    null,
    v_exercise.id,
    coalesce(v_exercise.name, v_type.name, 'Weekly challenge'),
    v_exercise.category,
...
-- Clear legacy source tags from existing feed rows.
update public.activities
set notes = null
where notes in ('Gang War', 'Weekly Challenge')
   or notes like 'Gang War · %'
   or notes like '% · Gang War'
   or notes like 'Weekly Challenge · %'
   or notes like '% · Weekly Challenge'
   or notes = 'Gang War · Weekly Challenge'
   or notes = 'Weekly Challenge · Gang War';
